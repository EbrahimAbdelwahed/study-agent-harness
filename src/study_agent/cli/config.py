"""Strict, non-secret configuration for the local reference repository."""
...
class LocalRepositoryConfig:
    ...
    def from_bytes(cls, payload: bytes) -> LocalRepositoryConfig:
        if type(payload) is not bytes or not payload or len(payload) > _MAX_CONFIG_BYTES:
        ...
        except (UnicodeError, json.JSONDecodeError, LocalConfigError) as error:
