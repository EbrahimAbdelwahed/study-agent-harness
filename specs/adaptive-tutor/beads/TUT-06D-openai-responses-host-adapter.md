...
- [x] Add one optional `openai` dependency extra; the base distribution retains
  zero runtime dependencies. Core, scripted host, CLI offline paths, and public
  imports succeed when the SDK is absent.
- [x] All `openai` imports are lazy or confined to the optional technical host
  adapter package. No Agents SDK import or agent-owned loop/state is added.
- [x] Configuration requires an explicit model id and API-key environment
...
- [x] The model id is configurable and recorded only as sanitized technical
  invocation provenance. No model name changes study behavior, capability
  schemas, prompts, gateway authority, or host limits.
- [x] One Responses request contains only the versioned host instruction,
...
- [x] The adapter cannot call gateway start/resume, create execution context,
...
- [x] Provider errors map to closed safe adapter errors. Only documented
  ...
  outcomes.
- [x] API-key mode is the only supported live integration. ChatGPT subscription,
  ...
  unavailable.
- [x] Request/response fixtures prove provider output cannot inject authority,
