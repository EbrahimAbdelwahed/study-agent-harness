...
- [x] `DialogueGate` immutably binds a prior validator boolean and typed default
  response; invalid or forward/model/run-input references are rejected.
- [x] False writes the default, a fail-closed skipped receipt, and continues
  without returning suspended; true preserves existing CAS suspension/resume.
- [x] Definition fingerprints bind the gate and default response.
- [x] Inspection/recovery recomputes the gate and rejects condition, receipt,
  default-output, trace-shape, and payload tampering.
- [x] Existing unconditional dialogue bytes/behavior and old resumed receipts
  remain compatible.
- [x] No new outcome, store, planner, provider, tool, or canonical state owner.
