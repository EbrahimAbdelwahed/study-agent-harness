...
- [ ] Add `recall = ["fsrs==6.3.1"]` as an optional dependency only after a
...
- [ ] All `fsrs` imports and conversions live under a scheduling adapter
...
- [ ] The adapter constructs the scheduler with explicit reviewed configuration
...
- [ ] Adapter receipts report exact `implementation_id = "py-fsrs"` and
  `implementation_version = "6.3.1"`; an unexpected installed version fails
  closed before any canonical write.
- [ ] The adapter reconstructs scheduling from the complete core history in
...
- [ ] Core ratings map explicitly and exhaustively to the package rating enum;
  timezone-aware UTC conversion and due output normalization are pinned.
- [ ] Conformance fixtures compare the fake and FSRS implementations only on
...
- [ ] If package API or deterministic behavior cannot satisfy these gates
