"""Offline reference composition for the bounded adaptive tutor host.

Run with ``PYTHONPATH=src python examples/reference_tutor_host.py``.  The
...
def _capture_descriptor() -> HostFileDescriptor:
    registry = HostFileRegistry(
        _MemorySource(b"# Heart valves\nTrusted host fixture."),
        MemoryHostFileIdentity(),
        MemoryHostFileSnapshotStore(),
        _FixedClock(),
        timedelta(hours=1),
    )
    return registry.capture("fixtures/lesson.md", COURSE_ID, SESSION_ID, "Lesson notes")


def run_reference_demo(
    learner_entry: str = "I have ten minutes and need help with valves.",
) -> dict[str, object]:
...
def main() -> None:
    print(json.dumps(run_reference_demo(), sort_keys=True, default=list, indent=2))
